...
describe Gitlab::Gem::Help do
  ...
  describe '.change_help_output!' do
    before do
      @cmd = 'create_branch'
      @help_output = "Gitlab::Gem.#{@cmd}(4, 'new-branch', 'master')"
      @help_output_with_options = 'Gitlab::Gem.groups({ per_page: 3 })'
    end

    it 'returns a String of modified output' do
      described_class.change_help_output! @cmd, @help_output
      expect(@help_output).to eq("Gitlab::Gem.create_branch(4, 'new-branch', 'master')")
    end
    ...
    it 'formats options hash and return a String of modified output' do
      ...
      expect(@help_output_with_options).to eq('Gitlab::Gem.groups({ per_page: 3 })')
    end
  end
  ...
  describe '.namespace' do
    ...
    it 'returns the full namespace for a command' do
      ...
      expect(@namespace).to eq("Gitlab::Gem::Client::Tags.#{@cmd}")
    end
  end
end
