  ...
  describe '.validate_gitlab_ci_yml' do
    context 'when valid content' do
      before do
        ...
        @lint_response = Gitlab::Gem.validate_gitlab_ci_yml('{ "image": "ruby:2.6", "services": ["postgres"], "before_script": ["bundle install", "bundle exec rake db:create"], "variables": {"DB_NAME": "postgres"}, "types": ["test", "deploy", "notify"], "rspec": { "script": "rake spec", "tags": ["ruby", "postgres"], "only": ["branches"]}}')
      end
    ...
    context 'when invalid content' do
      before do
        ...
        @lint_response = Gitlab::Gem.validate_gitlab_ci_yml('Not a valid content')
      end
    ...
    context 'when without content attribute' do
      before do
        ...
        @lint_response = Gitlab::Gem.validate_gitlab_ci_yml(nil)
      end
