...
require 'gitlab/gem/cli_helpers'
module Gitlab
  module Gem
    # Defines constants and methods related to configuration.
    module Configuration
      # An array of valid keys in the options hash when configuring a Gitlab::Gem::API.
      VALID_OPTIONS_KEYS = %i[endpoint private_token user_agent sudo httparty].freeze

      # The user agent that will be sent to the API endpoint if none is set.
      DEFAULT_USER_AGENT = "Gitlab Ruby Gem #{Gitlab::Gem::VERSION}"
      ...
      def get_httparty_config(options)
        ...
        httparty = Gitlab::Gem::CLI::Helpers.yaml_load(options)
        raise ArgumentError, 'HTTParty config should be a Hash.' unless httparty.is_a? Hash

        Gitlab::Gem::CLI::Helpers.symbolize_keys httparty
      end
    end
  end
end
