...
module Gitlab::Gem::Help
  extend Gitlab::Gem::CLI::Helpers
  ...
  class << self
      ...
      @help_map ||= begin
        actions.each_with_object({}) do |action, hsh|
          key = client.method(action)
                      .owner.to_s.gsub(/Gitlab::Gem::(?:Client::)?/, '')
          hsh[key] ||= []
          hsh[key] << action.to_s
        end
      end
    ...
    # Returns full namespace of a command (e.g. Gitlab::Gem::Client::Branches.cmd)
    def namespace(cmd)
